...
Examples:
  ...
  %(prog)s https://github.com/owner/repo --force-rebuild
    ...
    parser.add_argument(
        "--no-stream",
        action="store_true",
        help="Disable streaming output (for scripting scenarios)"
    )
...
def interactive_qa_loop(kb: KnowledgeBase, repo_url: str, no_stream: bool = False):
    ...
    while True:
        try:
            ...
            if no_stream:
                ...
                full_answer = ""
                for chunk in kb.query_stream(user_input):
                    print(chunk, end='', flush=True)
                    full_answer += chunk
                
                print()
                print("-" * 40)
                print("📚 Answer generated using retrieved documents")
                print()
                
                # Add to conversation history
                kb.add_conversation_turn(user_input, full_answer)
...
def main():
    ...
    try:
        ...
        interactive_qa_loop(kb, args.repo_url, args.no_stream)
